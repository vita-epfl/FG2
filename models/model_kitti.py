...
from models.modules import DeepResBlock_desc
from models.modules import self_attention_kitti as self_attention
from models.modules import cross_attention_kitti as cross_attention
...
class CVM(nn.Module):
    def __init__(self, device, grd_bev_res, grd_height_res, sat_bev_res, grid_size_h, grid_size_v, temperature=0.1, embed_dim=1024, desc_dim=128):
    ...
    def forward(self, grd_feature, sat_feature, camera_k):
        ...
        grd_query, _, _ = self.grd_attention_cross1(grd_query, grd_value, camera_k)
        ...
        grd_query, _, _ = self.grd_attention_cross2(grd_query, grd_value, camera_k)
        ...
        grd_query, _, _ = self.grd_attention_cross3(grd_query, grd_value, camera_k)
        ...
        grd_query, _, _ = self.grd_attention_cross4(grd_query, grd_value, camera_k)
        ...
        grd_query, _, _ = self.grd_attention_cross5(grd_query, grd_value, camera_k)
        ...
        grd_query, height_index, grd_keep_index = self.grd_attention_cross6(grd_query, grd_value, camera_k)
        ...
        grd_query = grd_query.permute(0,2,1).view(bs, self.embed_dim, int(np.floor(self.grd_bev_res/2))+1, self.grd_bev_res)
        ...
        grd_keep_index = grd_keep_index.flatten(1)
        for b in range(bs):
            matching_score_original[b, :, ~grd_keep_index[b]] = -float('inf')
