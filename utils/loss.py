...
grid_size_h_vigor = config.getfloat("VIGOR", "grid_size_h")
grid_size_h_kitti = config.getfloat("KITTI", "grid_size_h")
...
def compute_infonce_loss(Rgt, tgt, matching_score, sat_indices_B_seleted, grd_indices_B_seleted):
    ...
    T_s2g[:,:2,:2] = Rgt
    T_s2g[:,:2,2] = tgt[:,0,:] / grid_size_h_vigor * grd_bev_res
    T_s2g[:, 2, 2] = 1
...
def compute_infonce_loss_kitti(Rgt, tgt, matching_score, sat_indices_B_seleted, grd_indices_B_seleted):

    B, num_pts_sat, num_pts_grd = matching_score.shape
    matches_row = matching_score.flatten(1)
    
    
    T_s2g = torch.zeros(B, 3, 3).to(matching_score.device)
    T_s2g[:,:2,:2] = Rgt
    T_s2g[:,:2,2] = tgt[:,0,:] / grid_size_h_kitti * grd_bev_res
    T_s2g[:, 2, 2] = 1
    ...
    for b in range(B):
        valid_cols = (~torch.isinf(matching_score[b, 0, :])).nonzero(as_tuple=True)[0]
        candidate_cols = matching_col_indices[b]  # assumed shape: [N]
        
        keep_index_b = torch.isin(candidate_cols, valid_cols)

        if keep_index_b.sum() > 0:
            selected_indices = torch.nonzero(keep_index_b).to(matching_score.device)[:,0]
            ...
            selected_matching_indices = unique_row_indicies*num_pts_grd + coorespond_col_indicies
            positives = torch.exp(torch.index_select(matches_row[b], 0, selected_matching_indices))
            demoninator = torch.sum(torch.exp(matching_score[b, unique_row_indicies, :]), dim=1)
    ...
    for b in range(B):
        if keep_index[b].sum() > 0:
            ...
            selected_matching_indices = coorespond_row_indicies*num_pts_grd + unique_col_indicies
            positives = torch.exp(torch.index_select(matches_row[b], 0, selected_matching_indices))
            demoninator = torch.sum(torch.exp(matching_score[b, :, unique_col_indicies]), dim=0)
